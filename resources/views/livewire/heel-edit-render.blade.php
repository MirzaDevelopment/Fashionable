<div class="flex flex-col items-center" x-data="{ open: false }"> <!--Frontend component for rendering present heel categories for chosen product-->
    ...
    <div class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 p-4" x-show="open" x-transition>
        ...
        <h3 class="mt-6">Change heel type (only available for shoe products):</h3>
        <!-- Showing heel categories -->
        <div class="mt-6 grid grid-cols-2 gap-2 gap-x-10 justify-items-center">
            @foreach ($heelsAll as $index => $name)
            @if((in_array($name->heel_type, $heelNames)))
            <p wire:click="$parent.HeelDeSelect('{{$name->heel_type}}')" class="{{in_array($name->heel_type, $heelDeSelect)? 'p-1 min-w-16  self-center bg-white  border border-gray-200 rounded-lg shadow' : 'p-1 min-w-16  self-center bg-[#EDE8D0] border border-gray-200 rounded-lg shadow'}}" style="cursor: pointer" wire:key="{{$name->id}}">{{$name->heel_type}}</p>
            @else
            <p wire:click="$parent.HeelSelect('{{$name->heel_type}}')" class="{{in_array($name->heel_type, $heelSelect)? 'p-1 min-w-16  self-center bg-sky-200 border border-gray-200 rounded-lg shadow' : 'p-1 min-w-16  self-center bg-white border border-gray-200 rounded-lg shadow'}}" style="cursor: pointer" wire:key="{{$name->id}}">{{$name->heel_type}}</p>
            @endif
            @endforeach
        ...
        <section class="mt-6">
            ...
            @elseif(session('errorHeels'))
            <!-- Failed insert message -->
            <div class="lg:col-span-2 lg2:col-span-1" x-data="{open:true}">
                <div class="text-[#721c24] rounded-md bg-[#f8d7da] p-2.5 justify-center" x-show="open" x-on:click.outside="open=false" x-transition>{{ session('errorHeels')}}</div>
            </div>
            @elseif(session('emptyHeels'))
            <!-- Failed insert message -->
            <div class="lg:col-span-2 lg2:col-span-1" x-data="{open:true}">
                <div class="text-[#721c24] rounded-md bg-[#f8d7da] p-2.5 justify-center" x-show="open" x-on:click.outside="open=false" x-transition>{{ session('emptyHeels')}}</div>
            </div>
            @elseif(session('errorHeel'))
            <!-- Failed insert message -->
            <div class="lg:col-span-2 lg2:col-span-1" x-data="{open:true}">
                <div class="text-[#721c24] rounded-md bg-[#f8d7da] p-2.5 justify-center" x-show="open" x-on:click.outside="open=false" x-transition>{{ session('errorHeel')}}</div>
            </div>
            @endif
        </section>
        ...
        <div class="mt-6">
            ...
            <x-primary-button wire:click="$parent.editHeels" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:loading.class="opacity-50" class="lg:col-span-2 justify-center col-start-2 lg2:col-start-4">
